    ...
    def chazhi(self):
        if self.split == "train_supervised":
            for file in self.files:
                image_A_path = os.path.join(self.root, 'A', file)
                image_B_path = os.path.join(self.root, 'B', file)
                image_A = np.asarray(Image.open(image_A_path), dtype=np.float32)
                image_B = np.asarray(Image.open(image_B_path), dtype=np.float32)
                label_path = os.path.join(self.root, 'label', file)
                label = np.asarray(Image.open(label_path), dtype=np.int32)
                self.images_A.append(image_A)
                self.images_B.append(image_B)
                self.l.append(label)

            print(self.images_A[0].shape)

            self.images_A,self.images_B,self.l= self.yuanlai(self.images_A, self.images_B, self.l)

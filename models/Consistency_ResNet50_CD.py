...
class Consistency_ResNet50_CD(BaseModel):
    ...
    def forward(self, A_l=None, B_l=None, target_l=None, A_ul=None, B_ul=None, target_ul=None, curr_iter=None, epoch=None):
        if not self.training:
            return self.main_decoder(self.encoder(A_l, B_l, None))
        ...
        a,target_l = self.encoder(A_l,B_l,target_l)
        # print(a.shape)
        output_l    = self.main_decoder(a)
        ...
        # Supervised loss
        if self.sup_type == 'CE':
            loss_sup = self.sup_loss(output_l, target_l.long(), temperature=self.softmax_temp) * self.sup_loss_w
        ...
        # If semi supervised mode
            ...
            # Get main prediction
            x_ul, _ = self.encoder(A_ul, B_ul, target_ul)
            ...
            return total_loss, curr_losses, outputs, target_l

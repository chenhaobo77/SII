    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx in tbar:
            if self.mode == 'supervised':
                ...
                A_ul, B_ul, target_ul = A_ul.cuda(non_blocking=True), B_ul.cuda(non_blocking=True), target_ul.cuda(
                    non_blocking=True)
            ...
            total_loss, cur_losses, outputs = self.model(A_l=A_l, B_l=B_l, target_l=target_l.long(), A_ul=A_ul,
                                                         B_ul=B_ul,
                                                         curr_iter=batch_idx, target_ul=target_ul.long(),
                                                         epoch=epoch - 1)
            ...
            if self.mode == 'supervised':
                ...
                tbar.set_description(
                    'T ({}) | Ls {:.2f} Lu {:.2f} Lw {:.2f} PW {:.2f} IoU(change-l) {:.2f} IoU(change-ul) {:.2f}|'.format(
                        epoch, self.loss_sup.average, self.loss_unsup.average, self.loss_weakly.average,
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (A, B, target) in enumerate(tbar):
                ...
                tbar.set_description(
                    'EVAL ({}) | Loss: {:.3f}, PixelAcc: {:.2f}, IoU(no-change): {:.2f}, IoU(change): {:.2f} |'.format(
                        epoch,
